...
function Component() {
  ...
  const LatestPropertyStatusUpdate = async (id, e) => {
    const { message, success, err } = await Request.LatestProperty(id, {
      latest_property: `${e}`,
    });
    apiFunction();
      ...
      render: (i, record) => {
        return (
          <Switch
            checked={i}
            onChange={(e) => {
              LatestPropertyStatusUpdate(record.id, e);
            }}
          />
        );
      },
  ...
  return (
    <>
      <Breadcrumb title="Select Latest Property" parent="Properties" />
      <Container fluid={true} className="container-fluid">
        <Row>
          <Col lg="12">
            <Card className="card">
              <CardHeader className="card-header pb-0">
                <h5>Latest Property Details</h5>
              </CardHeader>
